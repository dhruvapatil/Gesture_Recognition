...
def get_all_frames(dir_path, hand):
    ...
    for frame in frames:
        ...
        try:
            image = skimage.io.imread(image_path)[5:105, 5:105, :]
        except:
            continue
...
def train(hand):
    ...
    with open("/s/red/a/nobackup/cwc/skeleton/ChaLearn17/predictions/rgb/train_%s.p" % hand, "wb") as fd:
...
def eval():
    ...
    with open("/s/red/a/nobackup/cwc/skeleton/ChaLearn17/predictions/rgb/eval_%s.p" % hand, "wb") as fd:
        cPickle.dump(prob_dict, fd)
